...
public:
    NumberNode(int val) : _val(val) {
        cout << "Create Number Node : " << val <<endl;
    }
...
public:
    ...
    OperatorNode(pNode lchild, pNode rchild, char ch) : BaseNode(lchild, rchild) {
        cout << "Create OperatorNode : " << ch << endl;
        switch (ch) {
...
//        cout << vis1.result << obj->getType() << vis2.result << endl;
        return ;
    }
    int result;
};
