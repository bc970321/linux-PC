/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2019年3月23日 星期六 10时05分52秒
 ************************************************************************/
...
void output(linkedlist *l) {
    DBG ("[");
    DBG("index :%d length :%d\n", l->index, l->length);
    for (Node *node = l->head.next; node != NULL; node = node->next) {
        DBG("%s ", inet_ntoa(node->addr_client.sin_addr));
    }
    DBG("]\n");
    return ;
}
...
char *get_conf_value(const char *pathname, const char *key_name) {
    ...
    while ((read = getline(&line,&len,fp)) > 0) {
        ...
        value[tempvalue] = '\0';
        //DBG("strlen(ptr+1):%d %s", strlen(ptr+1),ptr+1);
    }  
    //DBG("strlen: %d, %s",strlen(value), value);
    //DBG("------");
    if (value == NULL) {
        DBG("%d %s is node",__LINE__, key_name);
        perror("value :");
    }
    return value;
}
...
void *func2(void *arg){
    while (1) {
        ...
        if (size > 0) {
            DBG("%s: %s", inet_ntoa(temp->addr_client.sin_addr), buffer);
        }
        close(temp->fd_client);
    }
    return NULL;
